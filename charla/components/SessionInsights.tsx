    ...
    <div className="flex flex-col gap-4 mb-5">
      ...
      <div className="flex items-center gap-2">
        <span
          ...
          style={{ backgroundColor: "var(--accent-muted)", color: "var(--accent)", letterSpacing: "0.05em" }}
        >
          ✨ AI Session Insights
        </span>
      </div>
          ...
          <div className="flex flex-wrap gap-2">
            {insights.struggled_with.map((item, i) => (
              <span
                ...
                style={{ backgroundColor: "rgba(245,166,35,0.10)", color: "var(--color-warning)", border: "1px solid rgba(245,166,35,0.25)" }}
              >
                {item}
              </span>
            ))}
      ...
      <div style={sectionCard}>
        <div className="flex items-center justify-between mb-3">
          ...
          {submitted && (
            <span
              ...
              style={{
                backgroundColor: score >= insights.quiz.length / 2 ? "rgba(62,207,142,0.12)" : "rgba(240,68,68,0.12)",
                color: score >= insights.quiz.length / 2 ? "var(--color-success)" : "var(--color-danger)",
              }}
            >
        ...
        <div className="flex flex-col gap-3">
          {insights.quiz.map((q, qIndex) => {
            ...
            return (
              <div key={qIndex} className="rounded-lg p-3.5" style={{ backgroundColor: "var(--bg-raised)", border: "1px solid var(--border-default)" }}>
                ...
                <div className="flex flex-col gap-2">
                  {options.map((option, oIndex) => {
                    ...
                    const prefix = String.fromCharCode(65 + oIndex);
                    ...
                    return (
                      ...
                      >
                        <span
                          ...
                          style={{ width: "20px", height: "20px", backgroundColor: "var(--bg-subtle)", color: "var(--text-tertiary)" }}
                        >
                          {prefix}
                        </span>
                        <span>{option}</span>
                        {submitted && isCorrect && <span className="ml-auto text-xs">✓</span>}
                        {submitted && isSelected && !isCorrect && <span className="ml-auto text-xs">✕</span>}
                      </button>
                    );
                  })}
                </div>
                ...
                {submitted && (
                  <div className="mt-3 text-xs leading-relaxed rounded-md p-2.5" style={{ backgroundColor: "var(--bg-overlay)", color: "var(--text-tertiary)", border: "1px solid var(--border-default)" }}>
