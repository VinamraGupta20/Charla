...
const FileUpload = ({ onExtract, label = "Upload PDF or Word file" }: FileUploadProps) => {
  ...
  const extractFromPdf = async (file: File): Promise<string> => {
    ...
    // Use the worker bundled with the npm package, not a CDN —
    // avoids version-mismatch failures between installed pdfjs-dist
    // and an externally hosted worker file.
  ...
  return (
    <div className="flex flex-col gap-2">
      <div
        ...
        style={{
          border: `1.5px dashed ${
            error ? "var(--color-danger)" : dragActive ? "var(--accent)" : "var(--border-default)"
          }`,
